...
def plot_trajectories(dataset_trajectories_list,
    ...
    for alg in algorithm_names:
        ...
        cell_text = []
        for k in table_score:
            cell_text.append([v for v in table_score[k].values()])
        cell_text = np.array(cell_text)
        ax[0].axis('off')
        ax[0].axis('tight')
        ax[0].table(cellText=cell_text, rowLabels=rows, colLabels=columns, loc='center')
        ...
        for dataset_idx, dataset_nm in enumerate(dataset_names):
            ...
            ax[dataset_idx + 1].set_title(plot_settings['datasets_titles'][dataset_nm])
