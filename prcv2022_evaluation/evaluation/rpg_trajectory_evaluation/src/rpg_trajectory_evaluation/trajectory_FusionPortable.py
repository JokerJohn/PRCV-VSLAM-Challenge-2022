...
class Trajectory_FusionPortable:
    ...
    def __init__(self,
                 ...
                 suffix='',
                 est_type='traj_est',
                 nm_gt='groundtruth/traj/dataset_name.txt',
                 nm_est='estimated/traj/dataset_name.txt',
                 nm_matches='estimated/traj/dataset_name_traj_match.txt',
        ...
        self.saved_results_dir = os.path.join(
            os.path.join(self.data_dir, Trajectory_FusionPortable.saved_res_dir_nm),
            self.dataset_short_name)
        if not os.path.exists(self.saved_results_dir):
            os.makedirs(self.saved_results_dir)

        self.cache_results_dir = os.path.join(self.saved_results_dir,
                                              Trajectory_FusionPortable.cache_res_dir_nm)
        ...
        self.start_end_time_fn = os.path.join(self.data_dir,
                                              'start_end_time.yaml')
        ...
        if len(preset_boxplot_distances) != 0:
            ...
            if not self.boxplot_pcts:
                self.boxplot_pcts = Trajectory_FusionPortable.default_boxplot_perc
    ...
    def load_data(self, nm_gt, nm_est, nm_matches):
        ...
        """
        if not os.path.exists(nm_gt) or not os.path.exists(nm_est):
    ...
    @staticmethod
    def remove_cached_error(data_dir, dataset_name='', suffix=''):
        print("To remove cached error in {0}".format(data_dir))
        suffix_str = Trajectory_FusionPortable.get_suffix_str(suffix)
        base_fn = Trajectory_FusionPortable.rel_error_cached_nm + suffix_str + '.pickle'
        Trajectory_FusionPortable.remove_files_in_cache_dir(data_dir, dataset_name, base_fn)
    ...
    @staticmethod
    def remove_files_in_cache_dir(data_dir, dataset_name, base_fn):
        rm_fn = os.path.join(data_dir, Trajectory_FusionPortable.saved_res_dir_nm, dataset_name,
                             Trajectory_FusionPortable.cache_res_dir_nm, base_fn)
        Trajectory_FusionPortable._safe_remove_file(rm_fn)

    @staticmethod
    def remove_files_in_save_dir(data_dir, dataset_name, base_fn):
        rm_fn = os.path.join(data_dir, Trajectory_FusionPortable.saved_res_dir_nm, dataset_name,
                             base_fn)
        Trajectory_FusionPortable._safe_remove_file(rm_fn)
    ...
    def compute_boxplot_distances(self):
        ...
        print("Use percentage {0} of trajectory length.".format(
            self.boxplot_pcts))
        print("Trajectory_FusionPortable length {0}".format(self.traj_length))
        self.preset_boxplot_distances = [
            self.truncate(pct * self.traj_length, 2)
    ...
    def align_trajectory(self):
        if self.data_aligned:
            print("Trajectory_FusionPortable already aligned")
            return
        print(Fore.RED +
    ...
    def write_errors_to_yaml(self):
        ...
        for dist in self.rel_errors:
            ...
            dist_fn = os.path.join(
                self.saved_results_dir, Trajectory_FusionPortable.rel_error_prefix +
    ...
    def compute_relative_error_at_subtraj_len(self,
                                              subtraj_len,
                                              max_dist_diff=-1):
        if max_dist_diff < 0:
    ...
    def get_relative_errors_and_distances(self,
                                          error_types=[
